...
FROM docker.io/eclipse-temurin:25-jre-alpine AS run-base-xtrade
...
FROM run-base-xtrade AS release-xtrade
...
FROM docker.io/eclipse-temurin:25-jre-alpine AS run-base-xcache
RUN --mount=type=cache,target=/var/cache/apk apk add curl
RUN adduser -S -h /var/lib/xcache xcache
USER xcache
WORKDIR /var/lib/xcache

FROM run-base-xcache AS release-xcache
RUN --mount=type=cache,ro,from=build,source=/home/luser/xtrade/xcache/build,target=/build tar -C /var/lib -xf /build/distributions/xcache.tar
ADD conf/application.properties .
ADD --chown=xcache:xcache https://repo1.maven.org/maven2/io/opentelemetry/javaagent/opentelemetry-javaagent/2.22.0/opentelemetry-javaagent-2.22.0.jar opentelemetry-javaagent.jar
ENV JAVA_OPTS=-javaagent:opentelemetry-javaagent.jar
ENV SPRING_CONFIG_LOCATION=application.properties
ENTRYPOINT "./bin/xcache"
HEALTHCHECK CMD curl 'http://localhost:8080/actuator/health' -X GET -H 'Accept: application/json'
EXPOSE 8080